---
name: dispatch
description: Dispatch a task to a specific fleet runtime. `/dispatch hermes <task>` routes to @hermes; likewise for olivier_mbp, kimiclaw, rbitr, claude. Resolves the intent via tipi's runtime-dispatch.yaml.
---
...
2. Validate slug against known runtimes: `hermes`, `olivier_mbp`, `kimiclaw`, `rbitr`, `claude`.
...
4. Resolve to MCP tool:
   ...
   - `rbitr` → `tipi-rbitr/dispatch_to_rbitr`
